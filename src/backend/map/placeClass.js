...
class Place {
    ...
    static async findSuggestedPlacesForGroup(group, wday, startH, startM) {
        let [weekdayIndex, startTimeIndex] = Schedule.timeToIndex(wday, startH, startM);
        let origins = [];
        for (let i=0; i<group.users.length; i++) {
            let user = await User.getUserById(group.users[i]);
            let currentTimestamp = Date.now();
            if (startTimeIndex > 0) {
                if (user.schedule.table[weekdayIndex][startTimeIndex-1].timeStamp <= currentTimestamp) {
                    origins.push({
                    ...
                    let meeting = await Meeting.getMeetingById(user.schedule.table[weekdayIndex][startTimeIndex-1].meetingId);
        ...
        for (let dest of destinations) {
            let tmpRes = await kakaoGetTraveltime(origins, dest.location.x, dest.location.y);
            let durationSum = 0;
        ...
        for (let i=0; i<sumArr.length; i++) {
            ...
            if (destinations[i].envs.includes("moderate") && sumArr[i] < moderateSum) {
...
export const placeConverter = {
    ...
    fromFirestore: (snapshot, options) => {
        ...
        return new Place(data.id, data.buildingName, data.envs, data.location, data.rooms);
    }
};
